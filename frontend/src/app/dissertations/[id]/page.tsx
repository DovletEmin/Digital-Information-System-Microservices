...
import { savedService, DissertationHighlight } from '@/services/savedService';
...
export default function DissertationPage() {
  ...
  const dissertationId = Number(params.id);

  const [dissertation, setDissertation] = useState<Dissertation | null>(null);
  const [loading, setLoading] = useState(true);
  const [isSaved, setIsSaved] = useState(false);
  const [authToken, setAuthToken] = useState<string | null>(null);
  const [highlights, setHighlights] = useState<DissertationHighlight[]>([]);
  ...
  useEffect(() => {
    if (!Number.isNaN(dissertationId)) {
      viewService.recordView('dissertation', dissertationId);
    }
  }, [dissertationId]);
  ...
  const fetchDissertation = async () => {
    try {
      setLoading(true);
      const data = await dissertationService.getById(dissertationId);
      setDissertation(data);
    } catch (error) {
      console.error('Failed to fetch dissertation:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const checkIfSaved = async () => {
    try {
      const saved = await savedService.checkIfDissertationSaved(dissertationId);
      setIsSaved(saved);
    } catch {
      // not authenticated
    }
  ...
  const loadHighlights = async () => {
    try {
      const data = await savedService.getDissertationHighlights(dissertationId);
      setHighlights(data);
    } catch {
      // not authenticated
    }
  ...
  const loadMyRating = async () => {
    try {
      const data = await ratingService.getMyRating('dissertation', dissertationId);
  ...
  const handleSave = async () => {
    try {
      ...
      if (isSaved) {
        await savedService.unsaveDissertation(dissertationId);
        setIsSaved(false);
      } else {
        await savedService.saveDissertation(dissertationId);
        setIsSaved(true);
      }
    } catch (error) {
  ...
  const handleTextSelection = () => {
    ...
    if (selection && selection.toString().length > 0) {
      ...
      const contentElement = document.getElementById('dissertation-content');
  ...
  const handleHighlight = async (color: string) => {
    ...
    try {
      if (selectedHighlightIds.length > 0) {
        await Promise.all(
          selectedHighlightIds.map((id) => savedService.updateDissertationHighlight(id, { color }))
        );
      } else {
        await savedService.createDissertationHighlight({
  ...
  const handleClearHighlight = async () => {
    ...
    try {
      await Promise.all(selectedHighlightIds.map((id) => savedService.deleteDissertationHighlight(id)));
      await loadHighlights();
      setShowColorPicker(false);
      setCurrentHighlight(null);
      setSelectedHighlightIds([]);
  ...
  const handleSubmitRating = async () => {
    ...
    try {
      if (!ensureAuth()) return;
      await ratingService.setRating({ contentType: 'dissertation', contentId: dissertationId, rating });
      setMyRating(rating);
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-2xl font-bold mb-4">Dissertasiýa tapylmady</h1>
          <button
            onClick={() => router.push('/dissertations')}
            className="text-primary hover:underline"
          >
            Dissertasiýalara dolan
          </button>
        </div>
      </div>
    );
      ...
      <div className="container-custom py-8">
        ...
        <article className="max-w-4xl mx-auto bg-white rounded-lg shadow-sm p-8">
          ...
          <div className="flex flex-wrap items-center gap-4 text-sm text-gray-600 mb-6 pb-6 border-b">
            <div className="font-medium">{dissertation.author}</div>
            {dissertation.authors_workplace && (
              <>
                <span>|</span>
                <div>{dissertation.authors_workplace}</div>
              </>
            )}
            {dissertation.publication_date && (
              <>
                ...
                <span>|</span>
                <div>{formatDate(dissertation.publication_date)}</div>
              </>
            )}
            {dissertation.categories && dissertation.categories.length > 0 && (
              <>
                <span>|</span>
                <div className="text-primary">{dissertation.categories[0].name}</div>
              </>
            )}
          </div>

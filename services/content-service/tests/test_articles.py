...
def test_get_articles(client, test_article):
    ...
    items = data["items"]
    assert len(items) >= 1
    assert any(article["id"] == test_article.id for article in items)
...
def test_filter_articles_by_language(client, test_article):
    ...
    data = response.json()
    items = data["items"]
    assert all(article["language"] == test_article.language for article in items)
...
def test_filter_articles_by_type(client, test_article):
    ...
    data = response.json()
    items = data["items"]
    assert all(article["type"] == test_article.type for article in items)


def test_filter_articles_by_category(client, test_article, test_category):
    response = client.get(f"/api/v1/articles?category_id={test_category.id}")
    assert response.status_code == status.HTTP_200_OK
    data = response.json()
    assert "items" in data
...
def test_increment_views(client, test_article):
    ...
    # First GET increments views
    response = client.get(f"/api/v1/articles/{test_article.id}")
    assert response.status_code == status.HTTP_200_OK

    # Second GET increments views again
    response = client.get(f"/api/v1/articles/{test_article.id}")
    data = response.json()
    assert data["views"] > initial_views


def test_create_article_invalid_data(client):
    response = client.post(
        "/api/v1/articles",
        json={
            "title": "New Article"
